...
//  makeinfh
//
//  Created by David W. Gero on 8/23/26.
//  Copyright (C) 2026 by David W. Gero.  All Rights Reserved.
//
//  Convert the executable-adjacent infinite.cmb catalogue into a
//  deterministic C header containing its packed keys.
//
...
static uint64_t parseline(char *line, const char *inputpath,
                          size_t linenumber)
{
...
static KeyArray readkeys(const char *inputpath)
{
    ...
    for (;;) {
        ...
        linenumber++;
        appendkey(&result, parseline(line, inputpath, linenumber));
    }
    errno = 0;
    if (fclose(input) != 0) {
