...
#if defined(_WIN32) || defined(_WIN64)
    const char *backslash = strrchr(infinitepath, '\\');
...
static int writeinfiniteheader(void) {
    ...
    int failed = fprintf(output,
        "/* Generated by ysearch5 from its never-ending set. Do not edit. */\n"
        "/*\n"
        "* infinite.h\n"
        "* Part of ysearch5\n"
        "* Copyright (C) 2026 by David W. Gero\n"
    ...
    if (!MoveFileExA(temppath, outputpath,
        ...
        fprintf(stderr, "failed to replace %s with error %lu\n",
                outputpath, (unsigned long)error);
        goto cleanup;
    }
#else
    errno = 0;
...
static void recordinfinitecombinator(uint_fast32_t bufferhead,
                                     const char *buffer) {
    size_t length = strlen(buffer);

    if ((length == 0) || (length > MAXBUF) ||
        (buffer[length - 1U] != 'x') || (contents[bufferhead] != 'x')) {
        fprintf(stderr, "cannot record malformed divergent combinator %s\n",
                buffer);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv) {
    ...
    readinfiniteh();
#else
    initializeinfinitecatalog(argv[0]);
#endif
...
#else
    int result = closeinfinitecatalog();
#endif
